...
def getGraphDiff(pair):
    dotFile_data_path = './testDotFiles/'

    # Split the pairs to individual files
    file1 = pair.split(',')[0]
    file2 = pair.split(',')[1]

    # Get the name of the dotFile without its extension
    g1_name = file1.split('.')[0]
    g2_name = file2.split('.')[0]

    # Read the corresponding dot files
...
def main(): #main function from where the program starts
    ...
    with open('./testDotFiles.txt', 'r') as csvFile:
    ...
    for dotFile_i in dotFileList:
        for dotFile_j in dotFileList:
            if dotFileList.index(dotFile_j) >= dotFileList.index(dotFile_i):
                counter +=1
                pair = str(dotFile_i + ','+ str(dotFile_j))
                getGraphDiff(pair)

                print("\n Program status: \t" + str(counter) + " / " + str(int(totalGraphJsons)))
